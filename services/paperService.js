...
export const generatePaper = async (userId, subjectName, difficulty) => {
  ...
  const { systemPrompt, userPrompt } = buildPrompt(subject.contextCache, relevantChunks, difficulty);
  ...
  await GeneratedPaper.create({ userId, subject: subjectName, difficulty, paper, generatedAt: new Date() });
